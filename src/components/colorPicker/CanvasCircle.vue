...
<script>
    export default {
        ...
        mounted() {
            this.getParamColor('rgb(255,9,9)');
            this.initData();
            this.getDotList();
        },
        methods:{
            ...
            initData() {
                ...
                if (show.getContext) {//首先判断getcontext方法是否存在，这是一个良好的习惯
                    ...
                    // var gradientBar = context.createRadialGradient(show.width/2,show.height/2,0,show.width/2,show.height/2,140);
                    // var gradientBar = context.createRadialGradient(show.width,show.height/2,40,show.width,show.height/2,260);
                    var gradientBar = context.createRadialGradient(show.width,show.height/2,40,show.width,show.height/2,260);
                    gradientBar.addColorStop(0, '#ff0000');
                    gradientBar.addColorStop(1 / 5, '#ff00ff');
                    gradientBar.addColorStop(2 / 5, '#0000ff');
                    gradientBar.addColorStop(3 / 5, '#00ffff');
                    gradientBar.addColorStop(4 / 5, '#00ff00');
                    gradientBar.addColorStop(1, '#ffff00');
                    ...
                    show.addEventListener('click', function (e) {
                        ...
                        _this.$emit('getChildColor',rgbStr)
                        // _this.bus.$emit('updateColor',rgbStr)
                    });
                }
            },
            getParamColor(color) {
            ...
            getSelectColor() {
                console.log('getSelectColor',this.selectColor);
